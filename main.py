...
class Person(object):
	def __init__(self, name, str, dex, agi, vit, luk, classP, lvl):
		...
		self.classhit, self.classmhp, self.classflee, self.classcrit = 0, 0, 0, 0
		...
		self.mindmg = int(((self.str - (self.baseStr - 1)) + (
		self.str / 10) * 2 + self.adddmg + self.buffstat + self.luk / 5 + self.rweaponmindmg + self.lweaponmindmg * 0.5) * self.brokenArmMINdmg)
		...
		self.items, self.weapons, self.armor, self.pants, self.hat, self.shoes, self.rweapon, self.lweapon = {}, {}, {}, {}, {}, {}, {}, {}  # type: List[int]
		...
		if self.classP == 6:
			self.classp = "Maniac"
		self.maxhp = self.lvl * 2 + self.vit * 3 + 10 + self.str + self.classmhp
		self.hp = self.maxhp
	...
	def lvlUpConf(self):
		self.updateDmg()
		self.maxhp = self.lvl * 2 + self.vit * 3 + 10 + self.str + self.buffstat + self.buffhp + self.classmhp
		self.crit = (1 + self.luk / 3) + self.buffcrit if self.classP != 3 else (
																							1 + self.luk / 3 + 10) + self.buffcrit + self.crapcrit + self.classcrit
		self.hit = int((self.dex + 10 + self.dex / 15) * self.brokenArmHit) + self.buffhit + self.craphit + self.classhit
		self.flee = int(
			(self.agi + 5) * self.brokenLegFlee) + self.buffflee + self.crapflee if self.classP != 3 else int(
			(self.agi + 10) * self.brokenLegFlee) + self.buffflee + self.crapflee + self.classflee
		self.speed = (1 + self.agi / 10) + self.buffspeed
	...
	def buff(self):
		if self.marketban > 0:
			self.marketban -= 1
			if self.marketban == 0:
				print(pnt('sel', "Seems that you can return to a market - thing are quiet there now"))
		if self.crap > 0:
			self.crap -= 1
			if self.crap == 0:
		...
		if self.bufftime > 0:
			self.bufftime -= 1
			if self.bufftime == 0:
				player.str -= player.buffstat;
				player.dex -= player.buffstat;
				player.agi -= player.buffstat;
				player.vit -= player.buffstat;
				player.luk -= player.buffstat
	...
	def updateHitFlee(self):
		...
		self.flee = int(
			(self.agi + 5) * self.brokenLegFlee) + self.buffflee + self.crapflee if self.classP != 3 else int(
			(self.agi + 10) * self.brokenLegFlee) + self.buffflee + self.crapflee

	def updateDmg(self):
		self.mindmg = ((self.str - (self.baseStr - 1)) + (
					self.str / 10) * 2 + self.adddmg + self.buffstat + self.luk / 5 + self.rweaponmindmg + int(
			self.lweaponmindmg * 0.5)) * self.brokenArmMINdmg
		self.maxdmg = ((self.str + (
					self.str / 10) * 3 + self.adddmg + self.buffstat + self.luk / 5 + self.rweaponmaxdmg + int(
			self.lweaponmaxdmg * 0.5)) * self.brokenArmMAXdmg)
		...
		if self.parts2break != []:
			if rand <= 33:
				...
				if self.brokenarm == 0:
					...
					self.brokenPart()
					if inspect.stack()[1][3] == 'enmdmg': print(
						"[" + pnt('red', "HIT-50%s DMG-45%s") % (self.chancesign, self.chancesign) + "]")
					return 2
			elif rand > 67:
				if self.brokenjaw == 0:
					self.brokenjaw = 1
	...
	def isCrap(self):
		self.crap = 20
		self.crapflee += int(self.flee * 0.25);
		self.crapcrit += 5;
		self.craphit += int(self.hit * -0.4)
		self.lvlUpConf()

	def equip(self, item, hand):
		def bonus(part, hand, aORd):  # weapon or anrmor, hand, add or deduct bonus
			if part == 'w':  # for weapon
				if hand == 'r':
					whatBonus(self.rweapon.values()[0][3].keys()[0].lower(), self.rweapon.values()[0][3].values()[0],
							  'a' if aORd == 'a' else 'd')
				elif hand == 'l':
					whatBonus(self.lweapon.values()[0][3].keys()[0].lower(), self.lweapon.values()[0][3].values()[0],
							  'a' if aORd == 'a' else 'd')
				else:
					whatBonus(self.rweapon.values()[0][3].keys()[0].lower(), self.rweapon.values()[0][3].values()[0],
							  'a' if aORd == 'a' else 'd')
		...
		if item in self.weapons:
			if hand == 't':
				if self.weapons[item][6] == 1:
					...
					self.updateDmg()
					print("You equipped " + pnt('navy', "%s") % self.weapons[item][1] + " in " + pnt('sel',
																									 "two-hand mode"))
				else:
				...
				if self.lweapon == {}:
					...
					self.updateDmg()
					print("You equipped " + pnt('navy', "%s") % self.weapons[item][1] + " in the " + pnt('sel',
																										 "left hand"))
				else:
	...
	def about(self):
		print("Name:" + pnt('ital', self.name) + " " + pnt('sel', self.classp) + " Level:%d HP:%d/%d " % (
		self.lvl, self.hp, self.maxhp) + (pnt("green", pnt('sel', "on weed")) if self.bufftime > 0 else '') + " " + (
				  pnt("violet", pnt('sel', "market ban")) if self.marketban > 0 else '') + " " + (
				  pnt("red", pnt('sel', "broken jaw")) if self.brokenjaw == 1 else '') + " " + (
				  pnt("red", pnt('sel', "broken arm")) if self.brokenarm == 1 else '') + " " + (
				  pnt("red", pnt('sel', "broken leg")) if self.brokenleg == 1 else '') + " " + (
				  pnt("yellow", pnt('sel', "crapped")) if self.crap > 0 else '') + "\n-------------")
		print(
			"Stats:\nStr:%d  - weapon DMG and HP\t\tDMG: %d-%d\t%d%s attack chance:%d0%s\nDex:%d  - ability to hit opponent\tHIT: %d \nAgi:%d  - evasion rate\t\t\tFLEE:%d\tSPD:%d\nVit:%d  - HP and DMG absorbsion\t\tDEF: %d \nLuck:%d - CRIT & steal chance\t\tCRIT:%d\n--Money %d$\n--Exp %d/%d" % (
				self.str, self.mindmg, self.maxdmg, (2 + self.agi / 10),
				("nd" if self.agi / 10 == 0 else ("rd" if self.agi / 10 == 1 else "th")), self.agi % 10, self.chancesign,
				self.dex, self.hit, self.agi, self.flee, self.speed, self.vit, self.defence, self.luk, self.crit,
				self.money, self.exp, self.exp4lvl)
			+ "\n-------------")
		print("Equipment\nHat:%s\nArmor:%s\nRHand:%s\t\tLHand:%s\nPants:%s\nShoes:%s\n" % (
		pnt('sel', self.hat.values()[0][1]) if self.hat != {} else 'none',
		pnt('sel', self.armor.values()[0][1]) if self.armor != {} else 'none', pnt('sel', "+%d %s" % (
		self.rweapon.values()[0][7], self.rweapon.values()[0][1])) if self.rweapon != {} else 'none', pnt('sel',
																										  "+%d %s" % (
																										  self.lweapon.values()[
																											  0][7],
																										  self.lweapon.values()[
																											  0][
																											  1])) if self.lweapon != {} else 'none',
		pnt('sel', self.pants.values()[0][1]) if self.pants != {} else 'none',
		pnt('sel', self.shoes.values()[0][1]) if self.shoes != {} else 'none') + pnt('red', "Ammo:%d") % self.ammo)
...
class Enemy(Person):
	...
	def calculateDrop(self):
		...
		#print("chance:",chance)
		drop = random.randrange(1, int(chance))
		#print("drop:", drop)
		if drop <= 35 + player.luk:
			#print("calculating money...")
			#print("player.luk:",player.luk)
			self.dropmoney += 1 + self.toughenemy + self.lvl + random.randrange(int(player.luk if player.luk > 1 else 2)) + random.randrange(int((self.lvl * (1 + self.lvl / 10) + 5)))
			#print("dropmoney:",self.dropmoney)
			return 1
		elif drop <= 75:
			return 2
		elif drop <= 98 + player.luk * 2:
			#print("calculating consumables...")
			calculateCons()
			return 3
		elif drop <= chance:
			if self.lvl < 10:
	...
	def calculateExp(self):
		...
		if drop < 9:
			self.dropexp = ((self.lvl * (1 + self.lvl / 7)) * 1.5 + self.addexp) if self.lvl < 10 else (
						(self.lvl * (self.lvl / 5)) * 2.5 + self.addexp * 1.5)
		else:
			self.dropexp = ((self.lvl * (1 + self.lvl / 4)) * 2.2 + self.addexp * 1.5) if self.lvl < 10 else (
																														 self.lvl * (
																															 self.lvl / 3)) * 2.7 + self.addexp * 1.9

	def aboutEnemy(self):
		return pnt('navy', self.name) + " " + pnt('green', self.classp) + " " + "LVL:%d HP:" % self.lvl + pnt('red',
																											  "%d/%d ") % (
			   self.hp, self.maxhp) + (pnt("red", pnt('sel', "broken jaw")) if self.brokenjaw == 1 else '') + " " + (
				   pnt("red", pnt('sel', "broken arm")) if self.brokenarm == 1 else '') + " " + (
				   pnt("red", pnt('sel', "broken leg")) if self.brokenleg == 1 else '') + " " + (pnt("yellow",
																									 pnt('sel',
																										 "crapped")) if self.crap > 0 else '') + "\nStats: \nStr:%d  Dex:%d  Agi:%d  Vit:%d  Luck:%d  Drop:%d \nDMG:%d-%d CRIT:%d\nChance to hit:%d%s  Chance to flee:%d%s" % (
			   self.str, self.dex, self.agi, self.vit, self.luk, self.drop, self.mindmg, self.maxdmg, self.crit,
			   (player.hit + 50 - self.flee) if (player.hit + 50 - self.flee) < 100 else 100, self.chancesign,
			   ((self.hit + 50) - player.flee) if ((self.hit + 50) - player.flee) > 0 else 100, self.chancesign)
	...
	if item in player.consumable:
		if item == 'beer':
			if player.consumable[item] > 0:
				player.hp += player.lvl + 10; player.consumable[item] -= 1; print(
					"Bottle of beer gave you %dHP" % (player.lvl + 10))
			else:
				print("You're out of %s" % item)
		elif item == 'sunseeds':
			if player.consumable[item] > 0:
				player.hp += player.lvl + 5; player.consumable[item] -= 1;
				print(
					"Chowing down those seeds feels so good. +%dHP" % (player.lvl + 5))
			else:
				print("You're out of %s" % item)
		elif item == 'joint':
			if player.consumable[item] > 0:
				if player.bufftime == 0:
					player.buffstat = 2;
					print("Smoke weed every day.All stats+2,+%dHP" % (player.lvl + 5));
					player.str += player.buffstat;
					player.dex += player.buffstat;
					player.agi += player.buffstat;
					player.vit += player.buffstat;
			...
			if player.consumable[item] > 0:
				player.hp += player.lvl + 20; player.consumable[item] -= 1; print(
					"Mmm food of Gods - glorious kebab. +%dHP" % (player.lvl + 20))
			else:
				print("You're out of %s" % item)
	else:
...
def creator(classP, lvl):
	...
	classp = ''
	if classP == 1:
		str, dex, agi, vit, luk = int(stats * 0.24), int(stats * 0.24), int(stats * 0.24), int(stats * 0.24), int(
			stats * 0.08)
		classp = "Gangster"
		return str, dex, agi, vit, luk, classP, lvl
	elif classP == 2:
		str, dex, agi, vit, luk = int(stats * 0.4), int(stats * 0.15), int(stats * 0.03) + 1, int(stats * 0.4), int(
			stats * 0.02) + 1
		classp = "Bull"
		return str, dex, agi, vit, luk, classP, lvl
	elif classP == 3:
		str, dex, agi, vit, luk = int(stats * 0.1), int(stats * 0.25), int(stats * 0.35), int(stats * 0.06), int(
			stats * 0.28) + 1
		classp = "Hustler"
		return str, dex, agi, vit, luk, classP, lvl
	elif classP == 4:
		str, dex, agi, vit, luk = int(stats * 0.1), int(stats * 0.1), int(stats * 0.1), int(stats * 0.05), int(
			stats * 0.3)
		classp = "Junkie"
		return str, dex, agi, vit, luk, classP, lvl
	elif classP == 5:
		str, dex, agi, vit, luk = int(stats * 0.4), int(stats * 0.5), int(stats * 0.2) + 1, int(stats * 0.6), int(
			stats * 0.01) + 1
		classp = "Cop"
		return str, dex, agi, vit, luk, classP, lvl
	elif classP == 6:
		str, dex, agi, vit, luk = int(stats * 0.4), int(stats * 0.25), int(stats * 0.4), int(stats * 0.25), int(
			stats * 0.4)
		classp = "Maniac"
		return str, dex, agi, vit, luk, classP, lvl
...
def info(who, what):
	...
	if what in attrs:
		if who == 'pl':
			print("Player " + pnt('ital', what) + " is " + pnt('ital', "%s") % attrs[what])
		else:
			print("Enemy " + pnt('ital', what) + " is " + pnt('ital', "%d") % attrs[what])
	else:
...
# Event
def event():
	choice = input("What do you want to do? Type 'h' for help\n")

	def checkPlaces(place):
		if places[place] != 0:
			return 1
		else:
			print("You dont know where is %s located" % place); return 0
	...
	def whatDo(choice):
		...
		if choice == "w":
			...
			player.items.update({'rock': 6, 'hard rock': 6, 'ultra stone': 6, 'stone3000': 1});
			player.money = 2000;
			player.weapons = {'kn': ['w', 'Knife', 150, {'HIT': 3}, 3, 5, 0, 0],
							  'bk': ['w', 'Brass Knuckles', 50, '', 2, 4, 1, 0]};
			player.equip('kn', 1)
		elif choice == "ens":
			print(player.items)
		elif choice == "wep":
			...
			if player.admin == 0:
				player.admin = 1; print("Admin mode ON")
			else:
				player.admin = 0; print("Admin mode OFF")
		elif choice == "pl":
			...
			eqvar = choice.split()
			if (len(eqvar) > 2):
				player.equip(eqvar[1], eqvar[2])
			else:
				player.equip(eqvar[1], 1)
			...
			if checkPlaces('market'):
				if player.marketban == 0:
					market()
				else:
					print("People at market still searching for you - better not go to this place now")
		elif choice == 'sr':
		...
		# elif choice == "k": print("You're starting to swing your arms and legs. Peaple look at you like on idiot" )
			...
			print("Help\n---------\n'w' - Walk\t\t\t'sh' - Go to shop\t\t'sr' - Go to shooting range")
			print("'a' - Look at enemy\t\t'f' - Fight enemy\t\t\'gy' - Go to gym")
			print("'t' - Show statistics\t\t'q' - About you\t\t\t'v' - Go to veterinarian")
			print("'i' - Inventory\t\t'ma' - Market\t\tb/j/s/ke - Eat specific food (beer,joint,sunseeds,kebab)")
		elif choice == "exit":
			exit()
...
def walk():
	...
	if rand <= 7:
		...
		if enrand < 9:
			enemy = Enemy("Enemy", random.randrange(1, 5),
						  random.randrange(player.lvl if player.lvl <= 5 else player.lvl - 5, player.lvl + 6))
		else:
			enemy = Enemy("Enemy", random.randrange(5, 7),
						  random.randrange(player.lvl if player.lvl <= 5 else player.lvl - 3, player.lvl + 9))
		playstats.enemiesmet += 1
		if enemy.classP == 1:
			playstats.pats += 1
		elif enemy.classP == 2:
	...
	if enemy != '' and player.admin == 0:  # chance to evade fight
		choice = input("Opponent saw you. Fight him or try to run?\n f - fight	r - run	")
		if choice == 'r':
...
def found():
	...
	if rand <= 90 + player.luk:
		...
		calculateCons()
		print(pnt('violet', "Found consumable %s") % list(dropcons)[0])
...
def shop():
	...
	shopitems = {'beer': 20, 'bk': ['w', 'Brass Knuckles', 50, '', 2, 4, 1,
									0]}  # type,name,price,effects,mindmg,maxdmg,twohand?,refine
			...
			if type(shopitems[item]) is int:
				if player.money >= shopitems[item] * int(qty):
					if item not in player.consumable:
						...
						player.consumable[item] += int(qty)
	...
	def shoplist():
		for i in range(len(shopitems)):
			if type(shopitems.values()[i]) is int:
				print("%d - %s - %d$" % (i + 1, list(shopitems)[i], shopitems.values()[i]))
			else:
				print("%d - %s(DMG %d-%d) - %d$, shortcut:%s" % (
				i + 1, shopitems.values()[i][1], shopitems.values()[i][4], shopitems.values()[i][5],
				shopitems.values()[i][2], list(shopitems)[i]))
...
def gym():
	...
	def gymlist():
		print(
			"In gym you can increase your stats for cash\nTo increase specific stat type 'stat [how many times|max]'. Example: 'str 2' will increase your STR 2 times\n------")
		for i in range(len(gymmenu)):
			print("Stat: %s\tCost: %d$" % (list(gymmenu)[i], gymmenu.values()[i]))

	def choose():
		choice = input("You are in the gym: ")
		if choice == 'h':
			gymlist()
			return 1
			...
			if checkmoney(gymmenu[chvar[0]] * qty):
				if chvar[0] == 'str':
					print(pnt('red', "You pumped up some iron and got STR+%d" % qty));
					player.str += qty
				elif chvar[0] == 'agi':
					print(pnt('blue', "You runned for hours and got AGI+%d" % qty));
					player.agi += qty
				elif chvar[0] == 'vit':
					print(pnt('violet', "You did crossfit for hours and got VIT+%d" % qty));
					player.vit += qty;
				player.money -= gymmenu[chvar[0]] * qty
				gymmenu[chvar[0]] = int((1 + float(qty) / 10) * gymmenu[chvar[0]])
			else:
				print("You dont have money for that - " + pnt('red', '%d$') % player.money + "/" + pnt('green',
																									   '%d$') % int(
					gymmenu[chvar[0]] * qty))
			player.lvlUpConf()
			return 0
...
def shootingRange():
	...
	def choose():
		choice = input("You are on the shooting range: ")
		if choice == 's':
			if player.ammo > 0:
				player.ammo -= 1
				player.dex += 1
			...
			print("'s' - to train at shooting. Ammo " + (pnt('red',
...
def market():
	global player, enemy
	print(
		"'l' - list of goods\t'buy [what] #' - buy something. Example: to buy 5 sunseeds type 'buy sunseeds 5'\n's' - pickpocket\t'g' - play games of chance")
	shopitems = {'sunseeds': 10, 'kn': ['w', 'Knife', 150, {'HIT': 3}, 3, 5, 0, 0], 'ammo': 50,
				 'aj': ['a', 'Adidas jacket', 500, '', 1]}
	...
	def steal():
		...
		if chance > rand:
			...
			print(
				pnt('sel', pnt('green', "You have gone through the pockets of the people and stole %d$")) % moneystole)
			...
			if player.agi / 2 + player.luk > pay:
				print(
					"Since you are lucky and agile bastard you successfuly run out of angry people\nYou should wait some time until things go quiet in the market")
				return 0
			else:
				print(pnt('ital',
						  "~~Hey, fucker! I will torn your hands so next time you will think twice before steal!"))
				enemy = Enemy("Enemy", random.randrange(1, 4), random.randrange(player.lvl + 1, player.lvl + 10))
				print("You spotted an enemy - " + pnt('sel', '%s LVL:%d') % (enemy.classp, enemy.lvl))
				fightstance()
		...
		if item in shopitems:
			if type(shopitems[item]) is int:
				if player.money >= shopitems[item] * qty:
					...
					print("You dont have enought money (" + pnt('red', '%d$') % player.money + "/" + pnt('green',
																										 "%d$") % (
									  shopitems[item] * qty) + ")")
			else:
				if player.money >= shopitems[item][2]:
					...
					print(
						"You dont have enought money (" + pnt('red', '%d$') % player.money + "/" + pnt('green', "%d$") %
						shopitems[item][2] + ")")
				if qty > 1: print("You dont need more than 1 of that item")
		return 1
		...
		for i in range(len(shopitems)):
			if type(shopitems.values()[i]) is int:
				...
				if shopitems.values()[i][0] == 'w':
					...
					print("%d - %s(DEF %d) - %d$, shortcut:%s" % (
					i + 1, shopitems.values()[i][1], shopitems.values()[i][4], shopitems.values()[i][2],
					list(shopitems)[i]))
...
def vet():
	...
	def choose():
		choice = input("Animal want: ")
		if choice == "h":
			cost = 50 * (player.brokenjaw + player.brokenleg + player.brokenarm)
			print("'he' - to heal %d$\t't' - to heal trauma 50$ for each broken part (" % player.maxhp + pnt('green',
																											 "%s$") % cost + ")")
			return 1
		elif choice == 'he':
			cost = player.maxhp
				...
				if player.rweapon.values()[0][7] in (0, 1, 2):
					if 'rock' in player.items and player.money >= cost:
						if player.items['rock'] >= player.rweapon.values()[0][7] + 1:
							...
							player.money -= cost
							player.updateDmg()
							print(pnt("ital", 'You successfully upgraded your weapon to ') + pnt('sel', '+%d') %
								  player.rweapon.values()[0][7])
							return 1
						else:
							print("You dont have enough rocks - %d/%d" % (
							player.items['rock'], player.rweapon.values()[0][7] + 1))
					else:
						print("You dont have enough materials or money:\n--rocks %d/%d\tmoney %d/%d" % (
						player.items['rock'] if 'rock' in player.items else 0, player.rweapon.values()[0][7] + 1,
						player.money, cost))
						return 0
					...
					if 'hard rock' in player.items and player.money >= cost:
						if player.items['hard rock'] >= player.rweapon.values()[0][7] % 3 + 1:
							...
							print("You dont have enough hard rocks - %d/%d" % (
							player.items['hard rock'], player.rweapon.values()[0][7] + 1))
					else:
						print("You dont have enough materials or money:\n--hard rocks %d/%d\tmoney %d/%d" % (
						player.items['hard rock'] if 'hard rock' in player.items else 0,
						player.rweapon.values()[0][7] % 3 + 1, player.money, cost))
						return 0
					...
					if 'ultra stone' in player.items and player.money >= cost:
						if player.items['ultra stone'] >= player.rweapon.values()[0][7] % 6 + 1:
							...
							print("You dont have enough ultra stones - %d/%d" % (
							player.items['ultra stone'], player.rweapon.values()[0][7] + 1))
					else:
						print("You dont have enough materials or money:\n--ultra stones %d/%d\tmoney %d/%d" % (
						player.items['ultra stone'] if 'ultra stone' in player.items else 0,
						player.rweapon.values()[0][7] % 6 + 1, player.money, cost))
						return 0
					...
					if 'stone3000' in player.items and player.money >= cost:
						if player.items['stone3000'] >= 1:
							...
							print("You dont have enough stone3000 - %d/1" % (
							player.items['stone3000'], player.rweapon.values()[0][7] + 1))
					else:
						print("You dont have enough materials or money:\n--stone3000 %d/1\tmoney %d/%d" % (
						player.items['stone3000'] if 'stone3000' in player.items else 0, player.money, cost))
				else:
					print("This weapon is fully upgraded")
			else:
				print("No weapon in the right hand")
		else:
			if hand == 'l':
				if player.lweapon != {}:
					...
					if player.lweapon.values()[0][7] in (0, 1, 2):
						if 'rock' in player.items and player.money >= cost:
							if player.items['rock'] >= player.lweapon.values()[0][7] + 1:
								...
								player.updateDmg()
								print(pnt("ital", 'You successfully upgraded your weapon to ') + pnt('sel', '+%d') %
									  player.lweapon.values()[0][7])
								return 1
							else:
								print("You dont have enough rocks - %d/%d" % (
								player.items['rock'], player.lweapon.values()[0][7] + 1))
						else:
							print("You dont have enough materials or money:\n--rocks %d/%d\tmoney %d/%d" % (
							player.items['rock'] if 'rock' in player.items else 0, player.lweapon.values()[0][7] + 1,
							player.money, cost))
							return 0
						...
						if 'hard rock' in player.items and player.money >= cost:
							if player.items['hard rock'] >= player.lweapon.values()[0][7] % 3 + 1:
								...
								print("You dont have enough hard rocks - %d/%d" % (
								player.items['hard rock'], player.lweapon.values()[0][7] + 1))
						else:
							print("You dont have enough materials or money:\n--hard rocks %d/%d\tmoney %d/%d" % (
							player.items['hard rock'] if 'hard rock' in player.items else 0,
							player.lweapon.values()[0][7] % 3 + 1, player.money, cost))
							return 0
						...
						if 'ultra stone' in player.items and player.money >= cost:
							if player.items['ultra stone'] >= player.lweapon.values()[0][7] % 6 + 1:
								...
								print("You dont have enough ultra stones - %d/%d" % (
								player.items['ultra stone'], player.lweapon.values()[0][7] + 1))
						else:
							print("You dont have enough materials or money:\n--ultra stones %d/%d\tmoney %d/%d" % (
							player.items['ultra stone'] if 'ultra stone' in player.items else 0,
							player.lweapon.values()[0][7] % 6 + 1, player.money, cost))
							return 0
						...
						if 'stone3000' in player.items and player.money >= cost:
							if player.items['stone3000'] >= 1:
								...
								print("You dont have enough stone3000 - %d/1" % (
								player.items['stone3000'], player.lweapon.values()[0][7] + 1))
						else:
							print("You dont have enough materials or money:\n--stone3000 %d/1\tmoney %d/%d" % (
							player.items['stone3000'] if 'stone3000' in player.items else 0, player.money, cost))
					else:
					...
					print("No weapon in the left hand")
...
class Stats(object):
	...
	def showStats(self):
		...
		self.enemiesmet, self.pats, self.bik, self.vor, self.moncol, self.walked)
...
def pnt(font, text):
	if font == 'brown':
		...
		return "\x1b[34;1m" + text + "\x1b[0m"
	elif font == 'violet':
		return "\x1b[35;1m" + text + "\x1b[0m"
	elif font == 'blue':
		return "\x1b[36;1m" + text + "\x1b[0m"
	elif font == 'grey':
		return "\x1b[37;1m" + text + "\x1b[0m"
	elif font == 'ital':
		return "\x1b[3;1m" + text + "\x1b[0m"
	elif font == 'undrl':
...
def config():
	...
	print
	print("1." + pnt('undrl', "Gangster") + "\nJust an average person in da hood\tStarting gift:" + pnt('blue',
																										" bottle of beer") + "\n-------------")
	print("2." + pnt('undrl', "Bull") + "\nNo need for brain with so much strength\tStarting gift:" + pnt('red',
																										  " higher HP&STUN chance") + "\n-------------")
	print("3." + pnt('undrl', "Hustler") + "\nLucky and agile bastard\t\t\tStarting gift:" + pnt('yellow',
																								 " higher CRIT&FLEE chance") + "\n-------------")
